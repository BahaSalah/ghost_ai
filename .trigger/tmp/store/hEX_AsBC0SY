...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../lib-J2FNWTSC.mjs")).default;
}
...
// trigger/design-agent.ts
var NODE_COLORS = ["#1F1F1F", "#10233D", "#2E1938", "#331B00", "#3C1618", "#3A1726", "#0F2E18", "#062822"];
var NODE_SHAPES = ["rectangle", "pill", "circle", "diamond", "hexagon", "cylinder"];
var actionSchema = external_exports.discriminatedUnion("type", [
  external_exports.object({ type: external_exports.literal("add-node"), id: external_exports.string(), label: external_exports.string(), shape: external_exports.enum(NODE_SHAPES), color: external_exports.string(), x: external_exports.number(), y: external_exports.number() }),
  external_exports.object({ type: external_exports.literal("move-node"), nodeId: external_exports.string(), x: external_exports.number(), y: external_exports.number() }),
  external_exports.object({ type: external_exports.literal("resize-node"), nodeId: external_exports.string(), width: external_exports.number(), height: external_exports.number() }),
  external_exports.object({ type: external_exports.literal("update-node"), nodeId: external_exports.string(), label: external_exports.string().optional(), color: external_exports.string().optional(), shape: external_exports.enum(NODE_SHAPES).optional() }),
  external_exports.object({ type: external_exports.literal("delete-node"), nodeId: external_exports.string() }),
  external_exports.object({ type: external_exports.literal("add-edge"), id: external_exports.string(), source: external_exports.string(), target: external_exports.string(), label: external_exports.string().optional() }),
  external_exports.object({ type: external_exports.literal("delete-edge"), edgeId: external_exports.string() })
]);
var designPlanSchema = external_exports.object({
  reasoning: external_exports.string(),
  actions: external_exports.array(actionSchema)
});
function isValidColor(c) {
  return NODE_COLORS.includes(c);
}
__name(isValidColor, "isValidColor");
function clamp(v, min, max) {
  return Math.max(min, Math.min(max, v));
}
__name(clamp, "clamp");
async function pushStatus(liveblocks, roomId, message, step) {
  const payload = { message, step, timestamp: Date.now() };
  try {
    await liveblocks.mutateStorage(roomId, ({ root }) => {
      ...
      feed.push(new LiveObject(payload));
    });
  } catch (e) {
...
Generate the actions to update the canvas.`;
      const result = await generateObject({
        ...
        system: systemPrompt,
        prompt: userMessage,
        experimental_repairText: /* @__PURE__ */ __name(async ({ text, error: error3 }) => {
          logger.log("Repairing invalid model output", { error: error3.message });
          try {
            const parsed = JSON.parse(text);
            if (parsed?.actions && Array.isArray(parsed.actions)) {
              const fixed = parsed.actions.map((a) => {
          ...
          switch (action.type) {
            case "add-node": {
              ...
              flow.addNode({
                id: action.id,
                type: "canvasNode",
                position: { x: clamp(action.x, 0, 1e4), y: clamp(action.y, 0, 1e4) },
            ...
            case "resize-node": {
              flow.updateNode(action.nodeId, {
                width: clamp(action.width, 40, 1e3),
                height: clamp(action.height, 30, 1e3)
              });
              break;
            }
            case "update-node": {
